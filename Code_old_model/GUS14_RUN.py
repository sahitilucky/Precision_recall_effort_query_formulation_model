...
class GUS14_RUN():
	...
	def use_session_information(self, session):
		for interaction in session.interactions[:-1]:
			#print ("Results length: ", len(interaction.results))
			#print ("Clicks: ", interaction.clicks)
...
for session in all_sessions:
	act_session = Session(session, robust_data_collection)
	...
	for (docid,score) in topic_rel_docs_scores:
		if (score > 0):
			try:
				...
				pass
	print ("Session number: ", session.session_num)
...
with open("../simulated_sessions/GUS14_RUN_outputs.txt", "a+") as outfile:
	outfile.write("QCM_SAT_CLICKS AVERAGE NDCG: \n")
	outfile.write(str(float(sum(NDCG))/float(len(NDCG)))+ " ")
	outfile.write(str(float(len(NDCG)))+" ")
	outfile.write("\n\n\n")
