...
class Session_operations():
    ...
    def get_all_queries(self, sessions, dopreprocess = False):
        ...
        for topic_num in sessions_topics:
            queries_list_act = [list(filter(lambda l: l.type =="reformulate", session.interactions)) for session in sessions_topics[topic_num]]
    ...
    def get_validation_set_divisions(self, sessions):
        ...
        for topic_num in queries_list_topics:
            ...
            fold2 = list(queries_list[int(float(len(queries_list))/float(4)): 2*int(float(len(queries_list))/float(4))])
            fold3 = list(queries_list[2*int(float(len(queries_list))/float(4)): 3*int(float(len(queries_list))/float(4))])
        ...
        return [(validation_set1,validation_set1_test) , (validation_set2,validation_set2_test) , (validation_set3,validation_set3_test) , (validation_set4,validation_set4_test)]

    def divide_topic_sessions(self,act_sessions):
        queries_list_topics = self.get_all_queries(act_sessions, dopreprocess=True)
        topic_sets = []
        topic_set_numbers = []
        for topic_num in queries_list_topics:
            queries_list = list(set([' '.join(query) for query in queries_list_topics[topic_num]]))
            validation_set1 = {}
            validation_set1[topic_num] = [query.split() for query in queries_list]
            topic_sets += [validation_set1]
            topic_set_numbers +=[topic_num]
        return topic_sets,topic_set_numbers

    def divide_user_sessions(self,act_sessions):
        user_ids = {}
        for session in act_sessions:
            user_ids[session.user_id] = 1
        sessions_topics = self.get_session_topics(act_sessions)
        queries_list_topics = {}
        dopreprocess = True
        for user_num in user_ids:
            queries_list_topics[user_num] = {}
            for topic_num in sessions_topics:
                queries_list_act = [list(filter(lambda l: l.type == "reformulate", session.interactions)) for session in
                                sessions_topics[topic_num] if session.user_id==user_num]
                queries_list_act = [interaction.query for session in queries_list_act for interaction in session]
                if dopreprocess:
                    queries_list_act = [preprocess(" ".join(query), lemmatizing=True).split() for query in queries_list_act]
                if queries_list_act!=[]:
                    queries_list_topics[user_num][topic_num] = queries_list_act
            # print (queries_list_topics[topic_num])

        user_sets = []
        user_set_numbers = []
        for user_num in queries_list_topics:
            validation_set1 = {}
            for topic_num in queries_list_topics[user_num]:
                queries_list = list(set([' '.join(query) for query in queries_list_topics[user_num][topic_num]]))
                validation_set1[topic_num] = [query.split() for query in queries_list]
            user_sets += [validation_set1]
            user_set_numbers += [user_num]
        return user_sets,user_set_numbers
...
class Evaluation_queries():
    ...
    def query_similarity_evaluation(self, onlyqueries = False, top_k = None, validation_set = False, act_session_queries = None, simtype= "jsim"):
        '''
        ...
        if (not onlyqueries):
            ...
            for topic_num in self.candidate_queries_topics:
                queries_list_sim_topics[topic_num] = [query[0] for query in self.candidate_queries_topics[topic_num][:int(top_k)]]
        ...
        for topic_num in queries_list_act_topics:
            if topic_num in queries_list_sim_topics:
                ...
                simulated_to_act_sim,act_to_simulated_sim,simulated_to_act_sim_list,act_to_simulated_sim_list = self.jaccard_similarity(queries_list_act_topics[topic_num], queries_list_sim_topics[topic_num], simtype)
    ...
    def jaccard_similarity(self, queries_list_act, queries_list_sim, simtype):
        ...
        #simulated_to_act_sim = [[jsim(q1, q2) for q2 in queries_list_act]for q1 in queries_list_sim]
        #act_to_simulated_sim = [[jsim(q1, q2) for q1 in queries_list_sim]for q2 in queries_list_act]
    ...
    def reformulation_similarity(self, top_k):
        ...
        for topic_num in sim_reformulated_queries:
            ...
            for (query1,query2) in sim_reformulated_queries[topic_num]:
                similarities = [(q1,jsim(q1.split(), query1)) for q1 in act_query1s]
                ...
                for q1,sim_score in similarities:
                    ...
                    similarities = [jsim(query2,act_query2) for act_query2 in act_reformulated_queries_list[topic_num][q1]]
                    avg_similarity = float(sum(similarities))/float(len(similarities))
                    wt_avg_similarity += sim_score*avg_similarity
                    wt_den += sim_score
                if wt_den == 0: wt_avg_similarity = 0   
                else: wt_avg_similarity = float(wt_avg_similarity)/float(wt_den)
    ...
    def recall_eval(self, formatted_results, topic_num, cutoff, corpus_ids = None):
        predict_rel_docs = {}
        for idx,result in enumerate(formatted_results):
            predict_rel_docs[result["docid"]] = idx
        try:
        ...
        for docid in act_rel_doc_dict:
            if act_rel_doc_dict[docid] != 0:
                act_rel_doc_dict_2[docid] = act_rel_doc_dict[docid]
        predicted_doc_ids = list(predict_rel_docs.keys())
        act_rel_doc_dict_2 = list(act_rel_doc_dict_2.keys())
        recall = len(list(set(predicted_doc_ids).intersection(set(act_rel_doc_dict_2))))
        if len(act_rel_doc_dict_2)!=0:
            recall = float(recall)/float(len(act_rel_doc_dict_2))
        else:
            recall = 0
        precision = len(list(set(predicted_doc_ids).intersection(set(act_rel_doc_dict_2))))
        if len(predicted_doc_ids)!=0:
            precision = float(precision) / float(len(predicted_doc_ids))
        else:
            precision = 0
    ...
    def QF_rec_prec_act_eval(self, topic_num, cutoff, corpus_ids = None):
        '''
        Evaluate the search results present in the Session dataset sessions.
        :param topic_num:
        :param cutoff:
        ...
        for session in self.act_session_topics[topic_num]:
            for interaction in session.interactions:
                recall,precision = self.recall_eval(interaction.results, topic_num, cutoff)
                recalls += [recall]
                precisions += [precision]
        #topic_recs_precs[topic_num] = (recalls,precisions)
        if len(recalls)!=0:
        ...
        if len(precisions)!=0:
            ...
            avg_precision =0
        #topic_avgs[topic_num] = (avg_recall,avg_precision)
        return recalls,precisions,avg_recall,avg_precision
...
if __name__ == "__main__":
    ...
    methods = ['CCQF_0.1_0.765_0.135_2_noef_0.9_0.98_big_idx','CCQF_0.1_0.765_0.135_2_noef_type2_nowsu_0.9_0.99_big_idx','CCQF_0.1_0.765_0.135_2_noef_reform_0.9_0.9_big_idx', 'CCQF_0.1_0.765_0.135_2_noef_reform_0.9_0.9_nowsu_big_idx', 'CCQF_0.1_0.765_0.135_2_noef_reform_type2_0.9_0.9_big_idx', 'CCQF_0.1_0.765_0.135_2_noef_no_reform_big_idx', 'CCQF_0.1_0.765_0.135_2_noef_0.9_0.95_big_idx', 'CCQF_0.1_0.765_0.135_2_noef_noqf_0.9_0.95_big_idx', 'CCQF_0.1_0.765_0.135_2_noef_noqf_0.9_0.9_big_idx', 'CCQF_0.1_0.765_0.135_2_noef_type2_nowsu_0.9_0.9_big_idx']
    ...
    '''
    dataset = "Session_track_2014"
    save_dir = "../simulated_sessions/Session_track_2014/recall_precision_hists"
    if not os.path.exists(save_dir): os.makedirs(save_dir)
    sys.stdout = open(os.path.join(save_dir, 'Session_track_2014_recall_precisions_out.txt'), 'w')  # CCQF_0.6_0.2_0.6_reform_0.9_0.9
    session_ops = Session_operations()
    act_sessions = session_ops.get_actual_sessions(dataset)
    ev = Evaluation_queries(act_sessions, dataset = dataset )
    topic_descs = read_topic_descs(dataset)
    for topic_num in ev.topic_rel_docs:
        print("topic", topic_num)
        cutoff = 10
        recalls,precisions,avg_recall,avg_precision = ev.QF_rec_prec_act_eval(topic_num, cutoff, corpus_ids = None)
        print ("TOPIC NUM:", topic_num, topic_descs[topic_num])
        print ("Average recall: ", avg_recall)
        print("Average precision: ", avg_precision)
        plt.clf()
        plt.rcParams.update({'figure.figsize': (7, 5), 'figure.dpi': 100})
        plt.hist(np.array(recalls), bins=50)
        plt.gca().set(title='Recall Histogram', ylabel='Frequency')
        #plt.legend(loc="best")
        plt.savefig(os.path.join(save_dir, "rechist_topic_num_" + str(topic_num) + ".png"),
                    facecolor='white',
                    edgecolor='none', bbox_inches="tight")
        plt.clf()
        plt.rcParams.update({'figure.figsize': (7, 5), 'figure.dpi': 100})
        plt.hist(np.array(precisions), bins=50)
        plt.gca().set(title='Precision Histogram', ylabel='Frequency')
        #plt.legend(loc="best")
        plt.savefig(os.path.join(save_dir, "prechist_topic_num_" + str(topic_num) + ".png"),
                    facecolor='white',
                    edgecolor='none', bbox_inches="tight")
