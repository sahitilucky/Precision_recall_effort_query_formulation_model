...
def postprocess_query(query, topic_desc):
    ...
    topic_desc,stemmed_reverse_words_topic = preprocess(topic_desc, stopwords_file, lemmatizing = True, get_stemmed_reverse_words = True)
    '''
    postprocess_words = {}
    for word in words:
...
class Session():
    ...
    def get_actual_session(self, session_xml, session_xml_dataset):
        ...
        if session_xml_dataset == "Session_track_2014":
            self.user_id = session_xml.getAttribute("userid")
        ...
        for inte_idx,interaction in enumerate(interactions):
            ...
            for click in clicked_items:
                ...
                if session_xml_dataset == "Session_track_2012":
                    ...
                    clicks += [(getText(results[rank-first_rank].getElementsByTagName(cluewebid)[0].childNodes),rank, time)]
                else:
                    rank = int(getText(click.getElementsByTagName("rank")[0].childNodes))
                    first_rank =  int(results[0].getAttribute("rank"))
                    clicks += [(getText(results[rank-first_rank].getElementsByTagName(cluewebid)[0].childNodes),rank, time)]
...
def read_bigram_topic_lm(dataset):
    ...
    for topic_num in topic_descs:
        ...
        for i in range(len(topic_desc) - 1):
            topic_bigram_prob[topic_num][topic_desc[i]+" "+topic_desc[i+1]] = float(topic_bigram_ct[topic_num][topic_desc[i]+" "+topic_desc[i+1]])/float(topic_unigram_ct[topic_num][topic_desc[i]])
...
def make_preprocess_robust_data(dataset):
    ...
    for docid in robust_data_collection:
        robust_data_collection_wo_stopwords[docid] = preprocess(robust_data_collection[docid], stopwords_file, lemmatizing = True)
...
def prec(sim_list1, act_list2):
    #print (list1, list2, len(set(list1).intersection(set(list2))), len(set(list1).union(set(list2))))
    return float(len(set(sim_list1).intersection(set(act_list2))))/float(len(set(sim_list1)))

def recall(sim_list1, act_list2):
    #print (list1, list2, len(set(list1).intersection(set(list2))), len(set(list1).union(set(list2))))
    return float(len(set(sim_list1).intersection(set(act_list2))))/float(len(set(act_list2)))
...
def main():
    ...
    i = 0
    for docid in clueweb_snippet_collection_2:
        clueweb_snippet_collection_2[docid] = preprocess(clueweb_snippet_collection_2[docid], stopwords_file, lemmatizing = True)
        i += 1
        if (i%100000 == 0):
            print (i)
